...
storage_routes = Blueprint('files', __name__)
...
def _prefix(team_id, project_id, upload_id):
    return f"teams/{team_id}/projects/{project_id or 'shared'}/uploads/{upload_id}"


@storage_routes.route('', methods=['GET'])
@storage_routes.route('/', methods=['GET'])
@require_auth
def list_files():
...
@storage_routes.route('/upload', methods=['POST'])
@require_auth
def upload_file():
...
@storage_routes.route('/upload/chunk', methods=['POST'])
@require_auth
def upload_chunk():
    if 'file' not in request.files:
    ...
    prefix = _prefix(resolved_team_id, project_id, upload_id)
    ...
    if chunk_index + 1 == total_chunks:
        ...
        file_record, upload_error = process_upload(bytes(assembled), filename, content_type, g.user['id'], resolved_team_id, project_id, folder_id)
        if upload_error:
            return error(upload_error, 400)
        for index in range(total_chunks):
...
@storage_routes.route('/<file_id>', methods=['GET'])
@require_auth
...
@storage_routes.route('/<file_id>/download', methods=['GET'])
@require_auth
...
@storage_routes.route('/<file_id>/signed-url', methods=['GET'])
@require_auth
...
@storage_routes.route('/<file_id>', methods=['DELETE'])
@require_auth
...
@storage_routes.route('/<file_id>/versions', methods=['GET'])
@require_auth
def file_versions(file_id):
    ...
    if not _membership(result.data['team_id']):
        return error('Not a team member', 403)
    versions = db.table('file_versions').select('*').eq('file_id', file_id).order('version_number', desc=True).execute()
    return success(data=versions.data or [])


@storage_routes.route('/<file_id>/version', methods=['POST'])
@require_auth
def create_version(file_id):
    ...
    payload = {'id': generate_id(), 'file_id': file_id, 'version_number': version_number, 'storage_path': version_path, 'created_by': g.user['id'], 'created_at': now_iso(), 'mime_type': content_type, 'file_name': file_obj.filename or result.data.get('name'), 'size_bytes': len(file_bytes)}
    try:
...
@storage_routes.route('/folders', methods=['GET'])
@require_auth
def list_folders():
    team_id = request.args.get('teamId') or request.args.get('team_id')
    project_id = request.args.get('projectId') or request.args.get('project_id')
...
@storage_routes.route('/folders', methods=['POST'])
@require_auth
def create_folder():
    ...
    if team_error:
        return team_error
    folder = {'id': generate_id(), 'team_id': resolved_team_id, 'project_id': data.get('project_id'), 'parent_id': data.get('parent_id'), 'name': data['name'], 'path': data.get('path'), 'created_by': g.user['id'], 'created_at': now_iso(), 'updated_at': now_iso()}
...
@storage_routes.route('/quota/<team_id>', methods=['GET'])
@require_auth
def quota(team_id):
    if not _membership(team_id):
        return error('Not a team member', 403)
